function [ pSpike, keepStruct ] = classifySpikes_keepWvfs(filename,channelsToKeep)
...
% network and returns the spike classifications
    ...
    while keepGoing
        ...
        keepIdx = ismember(spikes(:,1),channelsToKeep);
        keepSpikes = [keepSpikes; spikes(keepIdx,:)];
        keepWvfs = [keepWvfs, waveforms(:,keepIdx)];
        
        currKeep.spikes = spikes(keepIdx,:);
        currKeep.wvfs = waveforms(:,keepIdx);
        ...
        keep_pSpike = [keep_pSpike; layer2_out(keepIdx)];
    ...
    keepStruct.wvfs = keepWvfs;
    keepStruct.nev = keepSpikes;
    keepStruct.pSpike = keep_pSpike;
